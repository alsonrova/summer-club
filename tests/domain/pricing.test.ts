...
const args = (p: PromotionRule[], maintenant = VENDREDI_20H, estMembre = false) => ({
  ...
  promotions: p, now: maintenant, isMember: estMembre,
})
...
describe('resolvePrice', () => {
  ...
  it('applique une remise en montant fixe', () => {
    expect(resolvePrice(args([base({ type: 'fixed', valeur: 5000 })])).finalPrice).toBe(45000)
  })

  it('ignore une promotion inactive', () => {
    expect(resolvePrice(args([base({ actif: false })])).finalPrice).toBe(50000)
  })
  ...
  it('applique un happy hour pendant sa plage horaire', () => {
    ...
    expect(resolvePrice(args([p], VENDREDI_20H)).finalPrice).toBe(40000)
  })

  it('ignore un happy hour en dehors de sa plage horaire', () => {
    const p = base({ heureDebut: 20, heureFin: 22 })
    expect(resolvePrice(args([p], VENDREDI_23H)).finalPrice).toBe(50000)
  })
  ...
  it('ignore un happy hour un jour non couvert par le masque', () => {
    ...
    expect(resolvePrice(args([p], VENDREDI_20H)).finalPrice).toBe(50000)
  })

  it('ignore une promotion membre pour un visiteur non connecté', () => {
    expect(resolvePrice(args([base({ membresSeulement: true })])).finalPrice).toBe(50000)
  })

  it('applique une promotion membre pour un membre', () => {
    expect(resolvePrice(args([base({ membresSeulement: true })], VENDREDI_20H, true)).finalPrice).toBe(40000)
  })
  ...
  it('ne descend jamais sous zéro', () => {
    expect(resolvePrice(args([base({ type: 'fixed', valeur: 999999 })])).finalPrice).toBe(0)
  })
})
